# Event-Driven Notification Hub — Coding Standards: Meta (Skills, Environment, Branching)

> Part 2 of 6. Also loaded: `CODING_STANDARDS.md` (core AI discipline), `CODING_STANDARDS_TESTING.md` (core TDD), `CODING_STANDARDS_TESTING_LIVE.md` (mock policy + backend integration), `CODING_STANDARDS_TESTING_E2E.md` (E2E), `CODING_STANDARDS_DOMAIN.md` (deploy/security)
...
- **Building with a specific framework/library** → find the matching skill (Fastify, Drizzle, Vitest, Kafka, etc.)
...
- For Node tooling: `npm run`, `npx tsc`, `npx vitest run` work fine in PowerShell as long as no special characters are inlined.
...
- Tests always run against local dev services (Docker PostgreSQL + Redpanda) on `dev` branch.
